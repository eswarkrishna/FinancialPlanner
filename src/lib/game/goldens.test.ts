...
import { computeGameGoldens } from "../../test/fixtures/goldens/buildGameGoldens";
...
const goldens: Record<(typeof P0_GAME_PROFILES)[number], GameGoldenSnapshot> = {
...
describe("game golden fixtures (SPEC §10 #14, §15.2)", () => {
  it("matches every Tier P0 profile snapshot", () => {
    ...
    for (const [name, expected] of Object.entries(goldens) as Array<
      [(typeof P0_GAME_PROFILES)[number], GameGoldenSnapshot]
    >) {
